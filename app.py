...
def search_faq(query: str) -> Dict[str, Any]:
    """מבצע את לוגיקת החיפוש המקורית, עם הגנה מפני כפילויות, נטרול עיוות אינטנטים 
    ורשת ביטחון המציגה את השאלה מהלוגיקה שלא נבחרה בראש השאלות הקשורות."""
    ...
    # --- מסלול ב': פילבק (Fallback) לפאזי בלבד במידה ולא נקבעה תוצאה ---
    ...
    # ==========================================================
    # מנגנון רשת ביטחון: בניית רשימת שאלות קשורות (similar_questions)
    # ==========================================================
    seen_questions = set()
    seen_questions.add(chosen_question_text.strip())  # חסימת השאלה שנבחרה כראשית
    
    # 1. בדיקה האם המנוע שנבחר הוא סמנטי, והאם למנוע הפאזי יש הצעה סבירה ושונה
    if "סמנטי" in search_type:
        if fuzzy_question_text != "לא נמצאה התאמה" and best_fuzzy_score >= 60:
            f_q_clean = fuzzy_question_text.strip()
            if f_q_clean not in seen_questions:
                similar_questions.append(f_q_clean)
                seen_questions.add(f_q_clean)
                
    # 2. בדיקה הפוכה: אם נבחר מנוע פאזי, נדחף את ההצעה הסמנטית למקום הראשון (אם היא ריאלית)
    elif "פאזי" in search_type:
        if semantic_question_text != "לא נמצאה התאמה" and best_embed_score <= 1.20:
            s_q_clean = semantic_question_text.strip()
            if s_q_clean not in seen_questions:
                similar_questions.append(s_q_clean)
                seen_questions.add(s_q_clean)

    # 3. השלמת שאר השאלות הקשורות (עד מגבלה של 3) מתוך הלהיטים של המודל הסמנטי
    if unique_hits:
    ...
    answer_text += f"\n{semantic_prefix}שאלה מזוהה סמנטי: {semantic_question_text}"
