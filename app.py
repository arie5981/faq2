...
def search_faq(query: str) -> Dict[str, Any]:
    ...
    if embeddings_ready and faq_store:
        ...
        # שלב הבורר הלוגי המשופר - הגנה מפני עיוות פאזי
        if unique_hits and best_embed_score <= 0.45:
            # אם הסמנטי מצא התאמה מדהימה (קרוב ל-0), הוא מקבל עדיפות מוחלטת!
            result_item = copy.deepcopy(faq_items[unique_hits[0][2]])
            search_type = "סמנטי (זכות וטו)"
        ...
        if result_item:
            ...
            # אנחנו עוברים על הלהיטים של המנוע הסמנטי לפי סדר הקרבה שלהם
            for doc, score, idx in unique_hits:
                q_text = faq_items[idx].question.strip()
                
                # אם השאלה הזו היא לא השאלה שנבחרה כתשובה הראשית
                if q_text not in seen_questions:
                    similar_questions.append(q_text)
                    seen_questions.add(q_text)
                    
                # אנחנו רוצים לעצור ברגע שיש לנו 3 שאלות קשורות
